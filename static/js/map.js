    ...
    toolsControl.onAdd = function(map) {
        ...
        container.innerHTML = `
            ...
            <div style="margin-bottom: 15px;">
                ...
                <button id="measure-distance-btn" class="btn btn-sm btn-outline-info" style="width: 100%;">
...
function updateLayersOpacity(opacity) {
    ...
    // Update opacity for all data layers
    if (window.activeLayers) {
        window.activeLayers.forEach(layer => {
            if (layer.mapLayer) {
                layer.mapLayer.setStyle({ fillOpacity: opacity * 0.7 });
            }
        });
    }
    
    // Update opacity for current layer if it exists
    if (currentLayer) {
        currentLayer.setStyle({ fillOpacity: opacity * 0.7 });
    }
}
